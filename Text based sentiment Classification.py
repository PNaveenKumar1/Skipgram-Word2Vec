# -*- coding: utf-8 -*-
"""PALLEKONDA_NAVEEN_KUMAR_22915_assignment1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1W5FrlWmE--D1sQM8uX4uyUhq-EKsbU7g
...
positive_words = {}
for sen in df_positive.review:
  words = preprocessing(sen)
  # print(words)
  for word in words:
...
def extract_features(X):
    ...
    X_split = preprocessing(X)

    good_words = ['love', 'good', 'brilliant', 'fantastic', 'amazing', 'great','won',
                      'enjoyable', 'Todd', 'beautiful', 'ways', 'entertaining', 'DVD', 'unlike', 'meets', 'Billy', 'born', 'refreshing', 'inspired', 'freedom', 'yes', 'miss', 'solid', 'loss', 'Tony', '7', 'games', 'normally', 'edge', 'unexpected', 'Ryan', 'more', 'week', 'office', 'be', 'extraordinary', 'ensemble', 'perfect', 'hilarious', 'helped', 'was', 'warm', 'bit', 'pleasantly', 'masterpiece', 'reminded', 'liked', 'Great', 'moved', 'hilarious', 'delightful', 'power', 'enjoyable', 'relatively', 'powerful', 'glad', 'favourite', 'All', '7/10', 'perfectly', 'opening', 'physical', 'rare', 'excellent', 'system', 'appreciate', 'performance', 'there', 'it', 'underrated', 'real', 'great', 'Sure', 'Thank', 'episode', '8/10', 'funniest', 'plenty', 'amazing', 'Best', 'real', 'realistic', 'awesome', 'Dirty', 'reality', 'beautifully', 'awesome', '8', 'brutal', 'easy', 'emotions', 'hilarious', 'IS', 'nicely', 'perfect', 'surprisingly', 'amazing', 'wonderful', 'terrific', 'excellent', 'enjoyed', 'loved', 'subtitles', 'favorite', 'subtle', 'wonderfully']

    bad_words = ['hate', 'bad', 'horrible', 'awful', 'terrible', 'mess', 'frustating', 'frustatingly','shamelessly',
                 'waste', 'poorly', 'fails', 'worst', 'Unfortunately', 'lame', 'boring', 'poor', 'awful', 'awful', 'lacks', 'bad', 'dull', 'worse', 'pathetic', 'tedious', 'wasted', 'positive', 'redeeming', 'awful', 'cheap', 'problem', 'ridiculous', '4/10', 'dull', 'appeal', 'lousy', 'pointless', 'terrible', 'stupid', 'nothing', 'horrible', 'painful', 'bad', 'weak', 'boring', 'wrote', 'mediocre', 'badly', 'dialogue', 'annoying', 'garbage', 'unbelievable', 'industry', 'Seagal', 'falls', 'supposed', 'annoying', 'terrible', 'script', 'excuse', 'guess', 'wasting', 'insult', 'Grade', 'disappointment', 'naked', 'bad', 'amateurish', 'unintentionally', 'disappointed', 'worse', 'Baldwin', 'walked', 'avoid', 'basically', 'bother', 'forward', 'disappointing', 'race', 'save', 'cliché', 'dull', 'Why', 'regarding', 'attempt', 'mess', 'Puerto', 'First', 'cheesy', 'monster', 'disaster', 'unfunny', 'predictable', 'rented', 'Oh', 'Avoid', 'instead','childrens', 'lacking', 'crappy', 'bunch', 'store', 'hotel', 'OK', 'costume', 'worse', 'Father', 'wooden', 'promising']
...
def get_feature_weights():
    ...
    # ADD YOUR CODE HERE
    feature_weights['good_word_count'] = 1.0
    feature_weights['bad_word_count'] = -1.0
...
def word_to_idx_BOW(X_data, K=10000):
    ...
    top_n_values = sorted(vocab.items(), key=lambda x: x[1], reverse=True)[:K]

    vocab = dict(top_n_values)
...
def extract_features_BoW(X, word_to_idx):
    ...
    features = {key: 0 for key in word_to_idx.keys()}
    for word in X.split(" "):
...
def get_learnable_weights_BoW(X_data, Y_data, word_to_idx_BOW, extract_features_BoW):
    ...
    # ADD YOUR CODE HERE
    word_to_id = word_to_idx_BOW(X_data)

    feature_names = list(word_to_id.keys())
    all_features = []

    for input_example in X_data:
      feature = []
      feat_dict = extract_features_BoW(input_example,word_to_id)
...
def predict(X, feature_weights):
    ...
    score = 0
    features = extract_features_BoW(X,feature_weights)
    # Here we just multiply the feature value
    # with its corresponding weight and aggregate
    for feat_name, feat_value in features.items():
        score = score + feat_value * feature_weights[feat_name]
...
sorted_weights = dict(sorted(BoW_feature_weights_lr.items(), key=lambda item: item[1]))

most_negative_words = list(sorted_weights.keys())[:5]
most_positive_words = list(sorted_weights.keys())[-5:]
...
MAX_SEQUENCE_LENGTH = 400 # sentences with length more than this value truncated
...
class SkipGram_Model(nn.Module):
    ...
    def forward(self, inputs_):
        ...
        # ADD YOUR CODE HERE
        embedded = self.embeddings(inputs_)
...
model_dir = 'PALLEKONDA_NAVEEN_KUMAR_22915'
...
folder = "PALLEKONDA_NAVEEN_KUMAR_22915"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
word_embeddings = model.cpu().get_word_embedding()

# ADD YOUR CODE HERE
# change the directory name with your SAPname and SRno

# Save the embeddings to the folder
np.save('PALLEKONDA_NAVEEN_KUMAR_22915/word_embeddings.npy', word_embeddings)
...
def get_word_similarity(words):
  ...
  word2vec = model.get_word_embedding()
  indices_of_interest = [vocab[word] for word in words]

  w2v_similarity = cosine_similarity(word2vec)
  word2vec = word2vec[indices_of_interest][0:]
  w2v_similarity = w2v_similarity[indices_of_interest][:, indices_of_interest]
